...
class Candle {
  ...
  factory Candle.raw(DateTime t, double o, double h, double l, double c) =>
      Candle(t, o, h, l, c);
}
...
List<Candle> parseCsv(String csv) {
  ...
  for (int i = 1; i < lines.length; i++) {
    ...
    final t = DateTime.tryParse(p[0].trim()) ??
        DateTime.fromMillisecondsSinceEpoch(0);
    double toD(String s) => double.tryParse(s.trim()) ?? 0;
    out.add(Candle(t, toD(p[1]), toD(p[2]), toD(p[3]), toD(p[4])));
  }
  return out;
}
