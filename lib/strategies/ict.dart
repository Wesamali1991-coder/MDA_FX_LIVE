...
String analyzeICT(List<Candle> c){
  ...
  for(final k in last){ if(k.h>hi) hi=k.h; if(k.l<ll) ll=k.l; }
  final bias = last.last.c>last.first.c? "صاعد":"هابط";
  return "ICT: ميل $bias · نطاق ${lo.toStringAsFixed(5)}–${hi.toStringAsFixed(5)} · ${fvg.isEmpty? "لا FVG حديث": "آخر FVG #${fvg.last}"}";
}
